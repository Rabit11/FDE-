...
const STORY_TEMPLATES = [
  ...
  { pattern: /验收|测试|demo/i, stories: ['在线验收流程', '反馈收集与 Backlog 回流', '验收标准自动核对'] },
...
function buildContext(user) {
  ...
  const sprints = queries.getSprints();
  const users = queries.getUsers();
  return JSON.stringify({ metrics, activeSprint: sprints.find(s => s.status === 'active'), items: items.slice(0, 30), team: users.map(u => ({ name: u.name, role: u.role, dept: u.dept })), user }, null, 0);
}

async function copilotChat(question, user) {
  const context = buildContext(user);
  const llmAnswer = await llm.smartAnalyze(context, question);
  const answer = llmAnswer || generateLocalCopilotAnswer(question, user);
  queries.saveChat(user.id, 'user', question);
  queries.saveChat(user.id, 'assistant', answer);
  return { answer, engine: llmAnswer ? 'llm' : 'local' };
}
...
async function suggestAssignee(itemId) {
  ...
  const workloads = executors.map(u => ({
    name: u.name,
    count: queries.getItems({ assignee: u.name }).filter(i => !['done', 'backlog'].includes(i.status)).length,
  }));
...
async function analyzeRisks() {
  ...
  const activeSprint = queries.getSprints().find(s => s.status === 'active');
  if (activeSprint) {
    const sprintItems = items.filter(i => i.sprint_id === activeSprint.id);
    const totalPts = sprintItems.reduce((s, i) => s + (i.story_points || 0), 0);
    const donePts = sprintItems.filter(i => i.status === 'done').reduce((s, i) => s + (i.story_points || 0), 0);
    ...
    const completion = totalPts > 0 ? donePts / totalPts : 0;
    if (progress > 0.6 && completion < 0.4) {
      risks.push({ level: 'high', item: activeSprint.name, reason: `Sprint 已过 ${Math.round(progress * 100)}% 但仅完成 ${Math.round(completion * 100)}%`, suggestion: '考虑范围裁剪或延期 Sprint' });
    }
  }
...
async function generateRetro() {
  ...
  const content = `## Sprint Retro · AI 洞察报告

### 📈 数据回顾
- 完成 Story Points: ${metrics.donePoints} / ${metrics.totalPoints}
...
3. 验收反馈 48h 内必须进入 Backlog`;

  queries.saveInsight('retro', 'Sprint 回顾报告', content, 'info');
...
module.exports = { splitRequirement, splitRequirementSmart, generateStandupSummary, analyzeRisks, generateRetro, copilotChat, suggestAssignee, buildContext };
