...
${active.length > 5 ? '• WIP 过高（' + active.length + '），建议限制在制品数量' : ''}`;
  ...
  return { content, stats: { active: active.length, done: done.length, blocked: blocked.length } };
}
...
function analyzeRisks() {
  ...
  return { risks, content, count: risks.length };
}
...
module.exports = { splitRequirement, generateStandupSummary, analyzeRisks, generateRetro };
